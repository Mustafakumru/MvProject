using HostelProject.WebUI.Dtos.TeamDto;
...
namespace HostelProject.WebUI.ViewComponents.Default
{
    public class _TeamPartial :ViewComponent
    {
        ...
        public async Task<IViewComponentResult> InvokeAsync()
        {
            ...
            var responsemessage = await client.GetAsync("http://localhost:5147/api/Staff"); //apideki verileri alıyoruz responsemesage değişkenine
            if (responsemessage.IsSuccessStatusCode) //apiye başarılı bir şekilde ulaşıyorsak işleme başlıyoruz 
            {
                ...
                var values = JsonConvert.DeserializeObject<List<ResultTeamDto>>(jsonData);  //resultabotdto tipinde listeye dönüşüyor 
